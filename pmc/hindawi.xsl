...
<xsl:template match="//contrib-group">
    ...
    <xsl:template match="fig">
    	...
    	<div style="border:1px solid rgb(228,228,228);padding:10px;width:320px;text-align:center;">
					<img>
						<xsl:attribute name="src">
							<!-- rules for figure URLs -->
							<xsl:text>http://www.hindawi.com/journals/</xsl:text>
							
							<xsl:value-of select="translate(//journal-meta/journal-title-group/journal-title, $smallcase, $uppercase)" />
    	...
    	<div style="border:1px solid rgb(228,228,228);padding:10px;margin-top:10px;margin-bottom:10px;">
    		<xsl:apply-templates />
    	</div>
    </xsl:template>
    ...
    <!-- mixed-citation -->
    <!-- mkxture of plain text and marked-up elements -->
    <xsl:template match="mixed-citation | nlm-citation">
    	<xsl:choose>
       				...
       				<xsl:text>ctx_ver=Z39.88-2004&amp;rft_val_fmt=info:ofi/fmt:kev:mtx:journal</xsl:text>
						...
						<xsl:choose>
							<xsl:when test="contains(volume, ')')">
								...
								<xsl:text>&amp;rft.volume=</xsl:text><xsl:value-of select="$before"/>
								<xsl:text>&amp;rft.issue=</xsl:text><xsl:value-of select="$issue"/>
							</xsl:when>
							<xsl:otherwise>
                        		<xsl:text>&amp;rft.volume=</xsl:text><xsl:value-of select="volume"/>							
                        	</xsl:otherwise>
						</xsl:choose>
